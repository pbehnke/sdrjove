...
class top_block(grc_wxgui.top_block_gui):
	...
	def __init__(self,parent,source_filename,sink_filename,should_save_file):
		...
		#if the filename ends with ".sdrjove", then we are probably running in RA mode with a named pipe,
		#so keep the file repeat on, otherwise, we are in DA mode, so turn file repeat off.
		repeat=source_filename.endswith(".sdrjove")

		self.gr_file_source_0 = gr.file_source(gr.sizeof_short*1, str(source_filename), repeat)
		self.gr_float_to_complex_0 = gr.float_to_complex(1)
		self.gr_rms_xx_0 = gr.rms_cf(0.0001)
