...
void main_init(void)             // Called once at startup
{
	...
	//wordwide =0 to turn on fpga
   	EP2FIFOCFG = EP4FIFOCFG = EP6FIFOCFG = EP8FIFOCFG = 0;
	SYNCDELAY;

	//flip the fpga FET on
	if(USBIE&bmHSGRANT){
 		OED |= bmBIT7;
 		IOD |= bmBIT7; 
 	}

 	//wait for thet fpga to turn on and start generating a clock
 	delay(3000);
	

  	// set the slave FIFO interface to 48MHz, use the ifclk from the fpga, internally run at 48mhz, slave fifo mode
   	//IFCONFIG |= 0x40;
	IFCONFIG |= 0x40;
   	SYNCDELAY;
  ...
  /* for (i=1;i<5;i++)
   {
    ...
    remote_wakeup_allowed = FALSE;                 // Enable remote-wakeup
	...
	{ 
	    ...
	    switch (AlternateSetting)
	    {
	        ...
	        case Alt3_IsocIN:
	            ...
	            SYNCDELAY;
                EP4FIFOCFG = EP6FIFOCFG = EP8FIFOCFG = 0;
                SYNCDELAY;
		    	IFCONFIG = 0x43;
				SYNCDELAY;
